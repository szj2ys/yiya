...
describe("checkRateLimit", () => {
  ...
  it("should allow requests within limit", () => {
    ...
    const r1 = checkRateLimit("u1", "explain", {
      now,
      limits: { byFeature: { explain: 2 } },
    });
    const r2 = checkRateLimit("u1", "explain", {
      now,
      limits: { byFeature: { explain: 2 } },
    });
  ...
  it("should deny requests exceeding limit", () => {
    ...
    checkRateLimit("u1", "explain", { now, limits: { byFeature: { explain: 2 } } });
    checkRateLimit("u1", "explain", { now, limits: { byFeature: { explain: 2 } } });
    const r3 = checkRateLimit("u1", "explain", {
      now,
      limits: { byFeature: { explain: 2 } },
    });
  ...
  it("should reset after window expires", () => {
    ...
    const r1 = checkRateLimit("u1", "explain", config);
    const r2 = checkRateLimit("u1", "explain", config);
    expect(r1.allowed).toBe(true);
    expect(r2.allowed).toBe(false);

    current = t0 + 1001;
    const r3 = checkRateLimit("u1", "explain", config);
